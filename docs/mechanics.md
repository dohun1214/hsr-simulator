...
| 항목 | 값 | 근거 |
|---|---|---|
| 범용 피해 감소 0.9 → 1.0 | | 2.8, 실측 |
| 격파 피해 | 8.3 | 게임 데이터 + 자료 |
| 행동 지연 | **25%** | 자료 원문 "Delays the enemy's action by 25%" |
| 속성별 디버프 부여 | **기본 확률 150%** | 자료 원문 "150% base chance to apply a debuff" |

디버프는 기본 확률 150% 로 부여되므로 효과 저항이 아주 높지 않으면 사실상 확정이다.
우리 구현은 기존 디버프 확률 공식(5.4)에 그대로 태운다.
...
**[미확인]** 게임 데이터의 얽힘·속박 정의에는 `DisableAction` 플래그가 있는데,
자료와 게임 내 설명("행동 게이지가 감소한다")은 행동 지연만 말한다.
우리는 **행동을 막지 않고 지연만** 적용했다. 실측으로 확인이 필요하다.
...
| 속성 배수 / 최대 인성치 배수 | 자료 + 게임 데이터 구조로 교차검증 |
| 행동 지연 25% / 디버프 확률 150% | 자료 원문 |
| 격파 효과 7종의 이름 · 구조 · 태그 | 게임 데이터 |
| 격파 효과 7종의 수치 | 자료 (게임 데이터의 수치는 이름 해시 뒤에 숨어 있어 복원 실패) |
| 격파 시 0.9 → 1.0 | 자료 2곳 + 실측 |
| 인성치 회복 시점 / 얽힘·속박의 행동 불가 여부 | **미확인 — 실측 필요** |
