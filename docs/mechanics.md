...
### 8.3 격파 피해 공식의 나머지 **[미확인]**

다음 항들은 **어떤 자료에서도 찾지 못했다.**

```
격파 피해 = 기본 피해(레벨) x 속성 배수 x (1 + 약점 격파 특효)
            x 최대 인성치 배수 x DEF 배수 x RES 배수 x ...
```

- **속성 배수**: 속성마다 다르다고 알려져 있으나 (물리/화염이 크고 양자/허수가 작다)
  1차 자료를 찾지 못했다. KQM SRL 저장소에도 격파 문서가 없고,
  Prydwen·HoYoWiki·Fandom 모두 공식을 제시하지 않는다.
- **최대 인성치 배수**: 마찬가지로 근거 없음.

구현: `BreakDamageConfig` 로 **상수를 데이터로 분리**하고 기본값은 `None`(미설정)으로 둔다.
설정하지 않으면 격파 피해가 0 이며, 계산했다고 표시하지 않는다.

**해결 방법**: 실측이 가장 빠르다. 우리는 이미
공격자 레벨별 기본 피해(확정), 적 DEF·RES(확정), 데미지 파이프라인(검증 완료)을 가지고 있으므로,
**적 하나를 격파하고 격파 피해 숫자 하나만 측정하면 나머지 배수를 역산할 수 있다.**

### 8.4 격파 시 일어나는 일

| 항목 | 상태 |
|---|---|
| 범용 피해 감소 0.9 → 1.0 | **[확인됨]** docs/mechanics.md 2.8 |
| 격파 피해 발생 | **[확인됨]** — 수치는 8.3 참고 |
| 행동 지연 | **[미확인]** — 지연이 일어난다는 것은 알려져 있으나 정확한 비율의 1차 자료 없음 |
| 속성별 격파 효과 (열상/연소/감전/풍화/동결/얽힘/속박) | **[미확인]** — 존재는 확실, 수치 근거 없음 |

행동 지연은 `BreakConfig.action_delay` 로 분리하고 기본값 0.25 를 쓴다.
**이 값은 근거가 없으므로 실측 전까지 신뢰하면 안 된다.**

### 8.5 인성치 회복 **[미확인]**

격파된 적이 자기 턴을 마치면 인성치가 회복되고 격파가 풀린다고 알려져 있다.
1차 자료를 찾지 못했으므로 그렇게 구현하되 미확인으로 둔다.
...
| 격파 기본 피해 (레벨별) | 게임 데이터 |
| 격파 시 0.9 → 1.0 | 자료 2곳 + 실측 |
| 속성 배수 / 인성치 배수 / 행동 지연 / 회복 시점 | **근거 없음 — 실측 필요** |
